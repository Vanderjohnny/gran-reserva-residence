...
export const OVERVIEW = { pos: [-70, 42, 88], target: [2, 16, -14] };   // captured in the viewer (window.__app.capture())   // captured in the viewer (window.__app.capture())   // captured in the viewer (window.__app.capture())   // captured in the viewer (window.__app.capture())   // captured in the viewer (window.__app.capture())   // captured in the viewer (window.__app.capture())   // captured in the viewer (window.__app.capture())   // captured in the viewer (window.__app.capture())   // captured in the viewer (window.__app.capture())   // captured in the viewer (window.__app.capture())   // captured in the viewer (window.__app.capture())   // captured in the viewer (window.__app.capture())   // captured in the viewer (window.__app.capture())   // captured in the viewer (window.__app.capture())   // captured in the viewer (window.__app.capture())   // captured in the viewer (window.__app.capture())   // captured in the viewer (window.__app.capture())   // captured in the viewer (window.__app.capture())   // captured in the viewer (window.__app.capture())   // captured in the viewer (window.__app.capture())   // captured in the viewer (window.__app.capture())   // captured in the viewer (window.__app.capture())   // captured in the viewer (window.__app.capture())   // captured in the viewer (window.__app.capture())   // captured in the viewer (window.__app.capture())   // null = automatic framing of the site bounds; capture the real one in the viewer later
...
export const STATUS = {
  ...
  reserved: { hex: '#2b7de9', label: { en: 'Reserved', pt: 'Reservada' } },
  sold: { hex: '#f2c230', label: { en: 'Sold', pt: 'Vendida' } },
};
...
// User review 2026-09-13: floor cuts are clean (reveal: 'hide' = no ghost floors, the facade mullion hidden), the cut views
// (cut: metres below the finished floor of the next storey = walls sectioned to show the interiors — the 4o has a ceiling
// at 13.3 that would hide its plan otherwise; plan: true = humanised plan on the slab),
// are steeper (pitch ~35 deg) and the dusk view farther (~180 m). Unit codes start at 101 (tipo stop shows 503 on FLOOR_07).
// humanised plans laid on the slab during the tour (stops with plan: true): image corners in model metres (x east, y north),
// the image top = y1 (north). Footprints from the unit quadrants of build_rev01_from_skp.py: typical floor x -7.1..10.4,
// y 6.4..26.6 (plan 1379x1600 = 0.862 vs 0.866); 4o pavimento with the terraces x -10.4..10.4, y 6.4..28.6 (plan
// 1491x1600 = 0.932 vs 0.937). lift = metres above the bottom of the unit boxes (LEVELS): the finished floors sit 0.29 m
// higher (typical 23.51 for LEVEL 23.22, 4o interior 10.55 / terraces 10.53, probed with a Raycaster on the floor nodes).
export const FLOOR_PLANS_3D = [
  { floors: [3], file: 'assets/plans/planta_diferenciado.jpg', x0: -10.4, x1: 10.4, y0: 6.4, y1: 28.6, lift: 0.34 },
  { floors: [4, 5, 6, 7, 8, 9, 10, 11], file: 'assets/plans/planta_tipo.jpg', x0: -7.1, x1: 10.4, y0: 6.4, y1: 26.6, lift: 0.34 },
];
export const TOUR = [
  ...
  { id: 'lazer', floor: 2, reveal: 'hide', cut: 1.0, pos: [30, 46, 26], target: [2, 8, -16], title: { pt: 'Lazer · 3º pavimento', en: 'Leisure · 3rd floor' }, text: { pt: 'Salão de festas, espaço gourmet, academia, game center, brinquedoteca, pet place e playground. Toque nos badges 360° para entrar nos ambientes.', en: 'Party room, gourmet space, gym, game center, kids room, pet place and playground. Tap the 360° badges to step inside.' } },
  { id: 'terracos', floor: 3, reveal: 'hide', cut: 1.0, plan: true, pos: [-38, 44, 18], target: [-4, 12, -16], title: { pt: 'Terraços · 4º pavimento', en: 'Terraces · 4th floor' }, text: { pt: 'Quatro apartamentos diferenciados com terraços privativos de até 130 m² de área privativa, com pérgola e espaço gourmet ao ar livre.', en: 'Four differentiated apartments with private terraces, up to 130 m² of private area, pergola and outdoor gourmet corner.' } },
  { id: 'tipo', floor: 7, unit: '503', reveal: 'hide', cut: 1.0, plan: true, pos: [-36, 64, 22], target: [2, 27, -16], title: { pt: 'Pavimento tipo', en: 'Typical floor' }, text: { pt: 'Quatro apartamentos por andar, do 5º ao 12º: 2 suítes, lavabo, living integrado e sacada. Áreas privativas de 73 a 83 m². Abra o Tour 360° no painel.', en: 'Four apartments per floor, 5th to 12th: 2 suites, guest toilet, open-plan living and balcony. 73 to 83 m² of private area. Open the 360° tour in the panel.' } },
  { id: 'rooftop', floor: null, pos: [24, 78, 20], target: [1, 42, -16], title: { pt: 'Rooftop', en: 'Rooftop' }, text: { pt: 'Piscina com prainha, piscina infantil e lounge com vista para o mar e para a mata.', en: 'Beach-entry pool, kids pool and a lounge overlooking the sea and the forest.' } },
  { id: 'entardecer', time: 0.64, pos: [-110, 62, 132], target: [2, 16, -14], title: { pt: 'Entardecer', en: 'Dusk' }, text: { pt: 'Ao entardecer os apartamentos se acendem. Use o controle de iluminação na legenda para escolher a hora do dia.', en: 'At dusk the apartments light up. Use the lighting slider in the legend to pick the time of day.' } },
];
